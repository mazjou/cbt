...
router.get('/export', async (req, res) => {
  ...
  try {
    ...
    const [options] = await pool.query(`SELECT question_bank_id, option_label, option_text, option_image, is_correct FROM question_bank_options WHERE question_bank_id IN (${ph}) ORDER BY question_bank_id ASC, option_label ASC;`, pObj);
    const optMap = {};
    ...
    const getImageRef = (p) => { if (!p) return ''; const v = String(p).trim(); if (/^https?:\/\//i.test(v)) return v; return path.basename(v).replace(/^\d{10,13}_/, '') || ''; };
    const rows = questions.map((q, idx) => {
      ...
      return { 'question_text': qText, 'image': getImageRef(q.question_image), 'points': q.points || 1, 'correct': correct,
        'A': opts['A']?.text || '', 'B': opts['B']?.text || '', 'C': opts['C']?.text || '', 'D': opts['D']?.text || '', 'E': opts['E']?.text || '',
        'image_a': getImageRef(opts['A']?.image), 'image_b': getImageRef(opts['B']?.image), 'image_c': getImageRef(opts['C']?.image), 'image_d': getImageRef(opts['D']?.image), 'image_e': getImageRef(opts['E']?.image),
        'difficulty': q.difficulty || 'MEDIUM', 'subject': q.subject_code || q.subject_name || '', 'chapter': q.chapter || '', 'tags': q.tags || '' };
    });
    const wb = XLSX.utils.book_new();
    const ws = XLSX.utils.json_to_sheet(rows, { header: ['question_text','image','points','correct','A','B','C','D','E','image_a','image_b','image_c','image_d','image_e','difficulty','subject','chapter','tags'] });
    ...
    res.send(buffer);
  } catch (e) { console.error(e); req.flash('error', 'Gagal export: ' + e.message); res.redirect('/teacher/question-bank'); }
});
